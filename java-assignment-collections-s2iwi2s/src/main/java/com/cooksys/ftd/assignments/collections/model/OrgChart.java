...
public class OrgChart {
	private Set<Employee> duplicate;
	private Map<Manager, Set<Employee>>  fullHierarchy;
	
	public OrgChart() {
		duplicate = new HashSet<Employee>();
		fullHierarchy = new HashMap<Manager, Set<Employee>>();
	}
	...
	public boolean addEmployee(Employee employee) {
		...
		if (employee instanceof Worker && !employee.hasManager()) {
			return false;
		}
		if (employee instanceof Manager) {
			fullHierarchy.put((Manager) employee, new HashSet<Employee>());
		}
		...
		if (employee.hasManager()) {
			...
			Set<Employee> employees = fullHierarchy.get(manager);
	...
	public boolean hasEmployee(Employee employee) {
		return this.duplicate.contains(employee);
	}
	...
	public Set<Employee> getAllEmployees() {
		...
		fullHierarchy.keySet().forEach(m -> {
			employees.add(m);
			employees.addAll(fullHierarchy.get(m));
		});
	...
	public Set<Manager> getAllManagers() {
		Set<Manager> managers = new HashSet<>(fullHierarchy.keySet());
		return managers;
	}
	...
	public Set<Employee> getDirectSubordinates(Manager manager) {
		Set<Employee> employees = fullHierarchy.get(manager);
		return employees != null ? new HashSet<Employee>(employees) : new HashSet<Employee>();
	}
	...
	public Map<Manager, Set<Employee>> getFullHierarchy() {
		Map<Manager, Set<Employee>> copy = new HashMap<Manager, Set<Employee>>();

		fullHierarchy.keySet().forEach(m -> {
			copy.put(m, new HashSet<>(fullHierarchy.get(m)));
		});
		return copy;
	}
